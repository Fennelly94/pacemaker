...
public class ConnectedComponentImage {
	...
	/**
	 * Returns the origional image the way it looked when it was taken from
	 * google images
	 * 
	 * @return
	 */
	...
	public Picture binaryComponentImage() {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				Color col = binarisePicture.get(x, y);
				if (Luminance.lum(col) < thresholdPixelValue) {
	...
	public String sizeOfPic() {
		...
		String size = "The width of this image is " + width
				+ " pixels and the height is " + height + " pixels.";
		return size;
	}
